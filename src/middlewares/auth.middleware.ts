...
export const is_verified = async (req: IRequestUser, res: Response, next: NextFunction) => {
  ...
  if (req.user.is_verified === false) {
    return res
      .status(401)
      .send({ status: userKey.notVerified, message: t('not-verified'), mdKey: 'is_verified' });
  }
  next();
};
...
export const ensure_admin = async (req: IRequestUser, res: Response, next: NextFunction) => {
  ...
  if (req.user.is_verified === false) {
    return res
      .status(401)
      .send({ status: userKey.notVerified, message: t('not-verified'), mdKey: 'ensure_admin' });
  }
  if (req.user.role !== adminRole) {
    return res
      .status(401)
      .send({ status: responseKey.unauthorized, message: t('unauthorized'), mdKey: 'ensure_admin' });
  }
  next();
};
